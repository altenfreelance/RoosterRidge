...
function Music() {
  return (
    <div>
      <Helmet>
        <title>Rooster Ridge - Music</title>
      </Helmet>
      ...
      <div className="row">
        {albumCards.map((album) => (
          <div key={album.props.name} className="mb-5">{album}</div>
        ))}
      </div>
    </div>
  );
...
export const LiveAtCappysAlbumCard = () => (
  <AlbumCard
...
export const ChickenScratchAlbumCard = (showCreditsUrl = true) => (
  <AlbumCard
    front_img_url="/images/albums/chicken-scratch/CoverFront.jpg"
...
const albumCards = [LiveAtCappysAlbumCard(), ChickenScratchAlbumCard()];
